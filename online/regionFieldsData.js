with (helper)
{
	...
	var regionId = EnvironmentVariables.Instance.GetValue(bo.CreateParams.UserToken, "@REGION_ID");
	...
	var docID = serverCallInit.Parameter;
	...
	var custData = "select  " +
		...
		"	from region_dynamic  " +
		"	where table_id=7888500000206170 and text_1='-111' and region_id=-222 and tenant_id=-333";

	custData = custData.Replace("-111", docID);
	custData = custData.Replace("-222", regionId);
	custData = custData.Replace("-333", tenantId);
	...
	//var customerFieldsJson = JsonSerializeTable(customerFieldsTable);
	...
	for (var i = 0; i < custDataTable.Rows.Count; i++)
	{
		...
		if ((!System.String.IsNullOrEmpty(dataRow["FIELD_INIT"])) && (dataRow["FIELD_INIT"].Contains("/")))
		{
			...
			if ((table_name == "customer") || (table_name == "customer_open_fields"))
			{
				...
				query_data = "select " + "" + c1_name + "" + " as FIELD_INIT from " + table_name + " where customer_id=-111";
				query_data = query_data.Replace("-111", customerId);
			}
			else if (table_name == "employee")
			{
				query_para = employeeID;
				query_data = "select " + "" + c1_name + "" + " as FIELD_INIT from employee where employee_id=-111";
				query_data = query_data.Replace("-111", employeeID);
			}
			else if (dataRow["FIELD_INIT"].Contains("dynamic"))
			{
				var dyn_tab = table_name.split('__');
				var table_id = dyn_tab[1];
				query_data = "select " + "" + c1_name + "" + " as FIELD_INIT from customer_dynamic where table_id=-111 and customer_id=-222";
				query_data = query_data.Replace("-111", table_id);
				query_data = query_data.Replace("-222", customerId);
			}
